...
def create_batch(row, transformer):
    ...
    days_frame = pd.read_csv(futura_weather_file, delim_whitespace = True, skiprows = 3, header = 0)
#    print(days_frame.columns)
    days_frame['@DATE'] -= 1001
#    print(days_frame['@DATE'].values)
    days_frame['@DATE'] = pd.to_datetime(days_frame['@DATE'], unit = 'D', origin = pd.Timestamp(str(year) + '-01-01'))
#    print(days_frame['@DATE'].values)
